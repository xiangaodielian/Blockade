...
////// Emissive:
                ...
                float3 finalColor = emissive + ((((_DiffuseTex_var.rgb*_DiffuseTint.rgb)*(max(0,dot(normalDirection,lightDirection))*attenuation))+(pow(max(0,dot(lightDirection,viewReflectDirection)),exp2(_Glossiness))*_SpecularIntensity))*_LightColor0.rgb);
...
////// Lighting:
                ...
                float4 _DiffuseTex_var = tex2D(_DiffuseTex,TRANSFORM_TEX(i.uv0, _DiffuseTex));
                float3 finalColor = ((((_DiffuseTex_var.rgb*_DiffuseTint.rgb)*(max(0,dot(normalDirection,lightDirection))*attenuation))+(pow(max(0,dot(lightDirection,viewReflectDirection)),exp2(_Glossiness))*_SpecularIntensity))*_LightColor0.rgb);
